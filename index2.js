...
function searchMovies() {
  // Get the user's search input from the search box
  const searchInput = document.getElementById("searchInput").value.trim().toLowerCase();

  // Fetch the movie data from 'main.json'
  fetch('main.json')
    .then((response) => {
      return response.json();
    })
    .then((data) => {
      ...
      // Get the container where we'll display the movies
      const filmContainer = document.getElementById("film");

      // Clear any previously displayed movies
      ...
      if (filteredMovies.length === 0) {
        ...
        // Generate HTML elements for the filtered movies
        filteredMovies.forEach((movie, i) => {
          ...
          // Add a hover effect when you put your mouse over the thumbnail
          thumbnail.addEventListener("mouseover", () => {
            thumbnail.style.opacity = 0.8;
          });

          // Add a click event to open a detailed view of the movie
          thumbnail.addEventListener("click", () => {
            popupShowModal(movie);
          });

          // Add the thumbnail to the movie container
          movieContainer.appendChild(thumbnail);

          // Create a section for movie information
          let info = document.createElement("div");
          ...
          // Add a click event to the movie title to open the detailed view
          ...
          // Add the title and description to the info section
          info.appendChild(titleHeading);
          info.appendChild(descriptionParagraph);

          // Add the info section to the movie container
          movieContainer.appendChild(info);

          // Create a section for the movie status (translated or untranslated)
          let statusDiv = document.createElement("div");
          statusDiv.classList.add("movie-status");
          ...
          // Add the status section to the movie container
          movieContainer.appendChild(statusDiv);

          // Add the movie container to the film container
...
// Add an event listener to the search input to trigger the searchMovies function
document.getElementById("searchInput").addEventListener("input", searchMovies);

// Function to open a detailed view of a movie
function popupShowModal(movie) {
  // Create a popup dialog for the movie details
  let popup = document.createElement("dialog");
  popup.id = "pop";
  ...
  // Create a video player to watch the movie
  let video = document.createElement("video");
  ...
  // Create a button to close the detailed view
  let closeButton = document.createElement("button");
  closeButton.classList.add("btn", "btn-danger");
  closeButton.textContent = "Close ❌";
  closeButton.style.marginTop = "10px";
  ...
  // Add the movie details to the popup
  popup.appendChild(imga);
  popup.appendChild(div3);

  // Add the popup to the webpage and show it
  document.getElementsByTagName("body")[0].appendChild(popup);
  popup.showModal();
}
